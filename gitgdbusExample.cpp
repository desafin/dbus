...
/* 우리가 내보내는 D-Bus 서비스에 대한 인트로스펙션 데이터.
   이 XML은 서비스의 구조를 정의하며, 인터페이스, 메소드, 신호 및 프로퍼티를 포함합니다. */
...
/* ---------------------------------------------------------------------------------------------------- */
/* 메소드 호출 핸들러. 인터페이스에서 메소드가 호출될 때 이 함수가 호출됩니다.
   메소드 이름을 확인하고 각 메소드를 적절하게 처리합니다. */
static void
...
/* 'Title' 프로퍼티에 대한 전역 변수. 프로퍼티 getter와 setter에서 사용됩니다. */
static gchar *_global_title = NULL;
/* 데모를 위해 두 값 사이를 토글하는 불리언 변수. */
static gboolean swap_a_and_b = FALSE;
/* 프로퍼티 getter. 이 함수는 프로퍼티의 값을 가져오는 요청을 처리합니다.
   프로퍼티 이름을 확인하고 적절한 값을 반환합니다. */
static GVariant *
...
/* 프로퍼티 setter. 이 함수는 프로퍼티의 값을 설정하는 요청을 처리합니다.
   프로퍼티 이름을 확인하고 적용 가능한 경우 값을 설정합니다. */
static gboolean
...
/* for now */
/* 인터페이스에 대한 VTable. 메소드, 프로퍼티 get, 프로퍼티 set 요청을
   각각의 핸들러에 매핑합니다. */
...
/* ---------------------------------------------------------------------------------------------------- */
/* 타임아웃 콜백 함수. 이 함수는 주기적으로 호출되며 'Foo'와 'Bar' 프로퍼티의
   값을 바꾸고 신호를 발생시킵니다. */
static gboolean
...
/* ---------------------------------------------------------------------------------------------------- */
/* 서비스가 버스를 획득했을 때의 콜백. 이 함수는 D-Bus에 객체를 등록하고
   프로퍼티를 바꾸는 타임아웃을 시작합니다. */
static void
...
/* 서비스 이름을 획득했을 때의 콜백. 이 예제에서는 사용되지 않습니다. */
static void
...
/* 서비스 이름을 잃었을 때의 콜백. 이 경우 프로그램을 종료합니다. */
static void
...
/* 메인 함수. GDBus 시스템을 초기화하고, 서비스를 설정하며, 메인 루프를 실행합니다. */
int
